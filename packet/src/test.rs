...
fn push_pkt(pool: &PktsHeap, headroom: usize, v: &Vec<u8>, sz: usize, npart: usize) {
    let mut pkt = pool.pkt(headroom).unwrap();
    let mut i = 0;
    // Push one byte at a time
    while i < v.len() {
        if (i + sz) >= v.len() {
            assert!(pkt.append(&v[i..]));
            break;
        } else {
            assert!(pkt.append(&v[i..i + sz]));
        }
        i += sz;
    }
...
fn append_w_headroom(pool: &PktsHeap, headroom: usize, npart: usize) {
...
#[test]
fn append_test() {
    let pool = packet_pool("append_test");
    append_w_headroom(&pool, 100, 3);
}

#[test]
fn prepend_test() {
    let pool = packet_pool("prepend_test");
    let mut pkt = pool.pkt(100).unwrap();
    assert_eq!(pkt.headroom(), 100);
    let v: Vec<u8> = (0..100).map(|x| (x % 256) as u8).collect();
    assert!(pkt.prepend(&v[0..]));
...
#[test]
fn slice_test() {
    let pool = packet_pool("slice_test");
    let headroom = 100;
    let mut pkt = pool.pkt(headroom).unwrap();
    ...
    let (s, l) = slices[0];
    let p = &pkt.particle;
    assert_eq!(s, &p.slice()[headroom..p.tail]);
    assert_eq!(l, PARTICLE_SZ - headroom);
    let (s, l) = slices[1];
    let p = p.next.as_ref().unwrap();
    assert_eq!(s, &p.slice()[0..p.tail]);
...
#[test]
fn move_head_test() {
    let pool = packet_pool("move_head_test");
    // One particle test
    let headroom = 100;
    let available = PARTICLE_SZ - headroom;
    let mut pkt = pool.pkt(headroom).unwrap();
...
#[test]
fn l2_test() {
    let pool = packet_pool("l2_test");
    let mac: Vec<u8> = vec![1, 2, 3, 4, 5, 6, 1, 2, 3, 4, 5, 6];
    // One particle test
    let headroom = 100;
    let mut pkt = pool.pkt(headroom).unwrap();
    assert!(pkt.append(&mac[0..]));
...
#[test]
fn l3_test() {
    let pool = packet_pool("l3_test");
    let ip: Vec<u8> = vec![1, 2, 3, 4, 1, 2, 3, 4];
    // One particle test
    let headroom = 100;
    let mut pkt = pool.pkt(headroom).unwrap();
    assert!(pkt.append(&ip[0..]));
